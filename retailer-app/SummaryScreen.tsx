...
import { View, Text, StyleSheet, ActivityIndicator, TouchableOpacity, TextInput, RefreshControl, ScrollView, FlatList } from 'react-native';
...
export default function SummaryScreen() {
  ...
  const Card = ({ label, value, style }: { label: string; value: string | number; style?: any }) => (
      ...
      {loading ? (
        ...
        <View style={{ paddingHorizontal: 16 }}>
          <View style={styles.cardsRow}>
            <Card label="Revenue" value={`RWF ${(report.revenue || 0).toLocaleString()}`} style={styles.cardGreen} />
            <Card label="Cost" value={`RWF ${(report.cost || 0).toLocaleString()}`} style={styles.cardRed} />
          </View>
          <View style={styles.cardsRow}>
            <Card label="Profit" value={`RWF ${(report.profit || 0).toLocaleString()}`} style={styles.cardBlue} />
            <Card label="Loss" value={`RWF ${(report.loss || 0).toLocaleString()}`} style={styles.cardYellow} />
          </View>
          <View style={styles.cardsRow}>
            <Card label="Sales" value={report.salesCount || 0} />
            <Card label="Credit Sales" value={report.creditSalesCount || 0} />
            <Card label="Credit Purchases" value={report.creditPurchasesCount || 0} />
          </View>
